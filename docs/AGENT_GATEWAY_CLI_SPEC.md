...
The Postgres routed task-write helper verifies scoped `POST /api/agent-gateway/tasks` can create a task through the Postgres adapter only when `AGENTOPS_POSTGRES_WRITE_HTTP=1` and the token has `tasks:create`; it also checks absent tokens, missing scope, body/header cross-workspace attempts, cross-agent attempts, and non-allowlisted Gateway writes remain fail-closed.
