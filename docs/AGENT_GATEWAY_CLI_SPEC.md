...
The Postgres routed task/execution/evidence helper verifies scoped `POST /api/agent-gateway/tasks`, `POST /api/agent-gateway/tasks/:task_id/claim`, `POST /api/agent-gateway/runs/start`, `POST /api/agent-gateway/tool-calls`, `POST /api/agent-gateway/artifacts`, and `POST /api/agent-gateway/evaluations/submit` can create, claim, start work, and write tool/evaluation/artifact evidence through the Postgres adapter only when `AGENTOPS_POSTGRES_WRITE_HTTP=1` and the token has the matching scopes; it also checks absent tokens, missing scopes, body/header cross-workspace attempts, cross-agent and same-workspace intruder attempts, while Gateway approval requests, memory/audit writes, live-runtime routes, and broad admin mutations remain fail-closed until separately proven.
