...
The Postgres routed task/execution-start helper verifies scoped `POST /api/agent-gateway/tasks`, `POST /api/agent-gateway/tasks/:task_id/claim`, and `POST /api/agent-gateway/runs/start` can create, claim, and start work through the Postgres adapter only when `AGENTOPS_POSTGRES_WRITE_HTTP=1` and the token has the matching scopes; it also checks absent tokens, missing scopes, body/header cross-workspace attempts, cross-agent and same-workspace intruder attempts, and broader non-allowlisted Gateway evidence writes remain fail-closed.
