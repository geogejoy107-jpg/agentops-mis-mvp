...
- LOOP DRIVER: `agentops operator loop-driver --adapter hermes|openclaw` is the compact local multi-step driver for Hermes/OpenClaw/Codex. Without `--confirm-loop` it is read-only and returns adapter readiness/preflight evidence, the compact launch brief, a compact RECORD review snapshot, and policy. With `--confirm-loop` it re-reads adapter readiness and the brief before each step, delegates execution to `advance-loop --fast-control --confirm-advance`, records receipts/control-readback through the existing bounded runner, caps execution at five steps, re-reads the review snapshot after each step, and still refuses live/workflow/approval/external-write commands.
