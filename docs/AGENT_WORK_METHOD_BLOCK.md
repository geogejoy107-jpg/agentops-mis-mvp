...
- A runtime doctor tells Hermes, OpenClaw, Codex, and remote Agents whether the local MIS API, adapter readiness, confirm-run wall, prepared-action wall, worker fleet, handoff evidence chain, launch packet, and redaction boundary are usable before a live runtime is asked to execute.
...
- RUNTIME DOCTOR: `agentops operator runtime-doctor` is the first local loop check for Hermes/OpenClaw/Codex. It aggregates `operator health`, `worker readiness`, `worker fleet`, and `operator handoff` into gates for local MIS API health, Hermes/OpenClaw runtime availability, `--confirm-run`, prepared actions for external writes, remote worker freshness, launch packet availability, handoff/evidence-chain status, Codex supervision, and token/raw-content redaction. It returns copyable commands only; it never starts runtimes, executes tasks, writes ledgers, or exposes tokens.
- LAUNCH PACKET: `agentops operator loop-launch-packet` produces the next agent's machine-readable READ/PLAN/RETRIEVE/COMPARE/EXECUTE/VERIFY/RECORD packet from intake, safe knowledge metadata, repo-map localization, handoff, and an agent-plan draft; it omits snippets and raw content to avoid leaking secret-like strings. The RETRIEVE phase includes `agentops commander repo-map` plus a sanitized `sources.repo_map` block with paths, symbols, hashes, provenance, ranking proof, and no raw file bodies, so the draft's proposed files are grounded before execution. The packet also includes `evaluation_contract`, `audit_contract`, and `execution_chain` blocks covering Agent Plan verification, intake, targeted checks, bounded advance preview/confirm, verified plan-evidence manifests, loop-audit, Action Queue receipts, receipt evaluations, tamper-chain expectations, bounded runner policy, denied live/approval/worker actions, and required ledgers. Each execution-chain step carries current `step_status`, `blocked_reason` or `ready_reason`, `next_safe_command`, and `receipt_state`; `control_summary` then selects the current recommended step, command, verify command, receipt command, control mode, human/receipt requirement, and copy-only proof so Hermes/OpenClaw/Codex can decide whether to read, copy, verify, or wait for an explicit receipt without guessing from prose.
