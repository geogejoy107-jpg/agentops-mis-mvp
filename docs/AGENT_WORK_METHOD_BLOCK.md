...
- Agent loop handoff: `agentops operator agent-loop-handoff` or `GET /api/operator/agent-loop-handoff`
- Loop supervision: `agentops operator loop-supervision` or `GET /api/operator/loop-supervision`
...
- AGENT LOOP HANDOFF: `agentops operator agent-loop-handoff` and `GET /api/operator/agent-loop-handoff` are the compact multi-consumer intake read for Hermes/OpenClaw/Codex. They aggregate local current-code readiness, live Hermes/OpenClaw ledger proof, per-adapter `start-check`, compact `loop-launch-packet --brief`, Method Block `phase_commands`, `method_gate_ids`, and Codex supervisor commands into one JSON matrix. The server API requires `tasks:read` for scoped Agent Gateway tokens/sessions and remains workspace-bound. It is read-only, never executes server shell or live runtimes, never mutates ledgers, and marks raw prompt/response/content/token omission. A consumer can be `attention` while review or memory pressure exists, but `ready_for_handoff` remains the structural proof that the agent can continue by copying the named commands instead of guessing the loop protocol.
- LOOP SUPERVISION: `agentops operator loop-supervision` and `GET /api/operator/loop-supervision` are the pre-confirm read after handoff. They classify each Hermes/OpenClaw/Codex adapter as `ready_to_confirm`, `record_first`, `preview_only`, `attention`, or `blocked`, expose `can_preview_loop`, `can_confirm_bounded_loop`, `should_record_before_execute`, review/memory/approval pressure, gate status, and layered `safe_read_commands`, `preview_commands`, and `confirm_required_commands`. It is read-only and never runs `loop-driver`, workers, runtimes, approvals, shell commands, or ledger writes.
