...
- START CHECK: `agentops operator start-check --adapter hermes|openclaw|mock` is the first pre-task read for Hermes/OpenClaw/Codex. It composes local readiness, worker readiness, runtime doctor, live product-readiness, compact launch brief, `local_run_path`, service-control preview, Agent Plan boundary, and next commands into one copy-only packet. It may return `attention` when live binaries, credentials, or ledger proof are missing, but it still stays useful because it names the next safe command without starting runtimes, executing server shell, mutating ledgers, or exposing raw prompts/responses/tokens.
...
- LOOP DRIVER: `agentops operator loop-driver --adapter hermes|openclaw` is the compact local multi-step driver for Hermes/OpenClaw/Codex. Without `--confirm-loop` it is read-only and returns adapter readiness/preflight evidence, the compact launch brief, and policy. With `--confirm-loop` it re-reads adapter readiness and the brief before each step, delegates execution to `advance-loop --fast-control --confirm-advance`, records receipts/control-readback through the existing bounded runner, caps execution at five steps, and still refuses live/workflow/approval/external-write commands.
