...
| Zones route to formal MIS pages | Pass | `pixelModel.ts` maps zones to MIS routes such as `/admin`, `/workspace/agents`, `/workspace/tasks`, `/admin/connectors`, `/admin/toolcalls`, `/workspace/approvals`, `/admin/evaluations`, `/workspace/memory`, `/admin/audit`, `/admin/bases/notion`, `/admin/runs`, and `/admin/templates`. |
