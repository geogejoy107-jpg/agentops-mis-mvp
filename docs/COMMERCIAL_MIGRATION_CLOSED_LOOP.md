...
- `commercial_release_grade_receipt_recording_v1` is the operator recording
  preview after the rerun bundle. It materializes one Gate 1-5 recording request
  per receipt, with `preview_only_json_patch` operations for
  `COMMERCIAL_EVIDENCE_RECEIPTS.json`, command counts, patch-preview counts,
  ...
  and must remain blocked until operator confirmation, current receipt heads,
  exact-head CI, clean worktree, handoff, merge readiness, and real-runtime
