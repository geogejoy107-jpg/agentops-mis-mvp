...
| Postgres routed task/execution/evidence write | `AGENTOPS_STORAGE_BACKEND=postgres`, `AGENTOPS_POSTGRES_READ_ONLY_HTTP=1`, and explicit `AGENTOPS_POSTGRES_WRITE_HTTP=1` allow only `POST /api/tasks`, scoped `POST /api/agent-gateway/tasks`, scoped `POST /api/agent-gateway/tasks/:task_id/claim`, scoped `POST /api/agent-gateway/runs/start`, scoped `POST /api/agent-gateway/tool-calls`, scoped `POST /api/agent-gateway/artifacts`, and scoped `POST /api/agent-gateway/evaluations/submit` to write through `PostgresAdapter`; absent tokens, missing scopes, body/header cross-workspace, cross-agent, same-workspace intruder, Gateway approval requests, memory/audit writes, live-runtime routes, and broad admin mutations remain blocked | first real human/API plus Agent Gateway task-create, claim, run-start, and tool/evaluation/artifact evidence-write proof through the Postgres adapter without opening approval, memory, audit, live-runtime, or broad admin mutations | `python3 scripts/storage_postgres_http_write_task_smoke.py` |
