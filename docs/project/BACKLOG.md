...
| P1-05 | Split oversized horizontal modules | In Progress | Slices 1-4 extract runtime connector capability policy, registry row/upsert helpers, trust state read/update helpers, read-model cache behavior, and worker fleet status/fleet-view aggregation into `agentops_mis_runtime/capabilities.py`, `agentops_mis_runtime/connectors.py`, `agentops_mis_runtime/trust.py`, `agentops_mis_core/read_model_cache.py`, and `agentops_mis_core/worker_fleet.py`; server keeps HTTP routes, health probing, refresh orchestration, runtime/audit writes, auth checks, SQLite producers, local daemon/process discovery, runtime adapter readiness, and fleet hygiene mutations. Guarded by `scripts/module_boundary_smoke.py` plus runtime capability/readiness/trust, read-model cache, and worker status/fleet smokes; continue with small strangler slices only |
