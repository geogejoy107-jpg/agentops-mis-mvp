...
| P1-05 | Split oversized horizontal modules | In Progress | Slice 1 extracts runtime connector capability policy plus registry row/upsert helpers into `agentops_mis_runtime/capabilities.py` and `agentops_mis_runtime/connectors.py`, preserves `GET /api/runtime-connectors` / `agentops runtime connectors` / worker readiness contracts, and is guarded by `scripts/module_boundary_smoke.py` plus runtime capability/readiness smokes; continue with small strangler slices only |
