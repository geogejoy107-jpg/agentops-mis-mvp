...
| ID | Work item | Status | Acceptance evidence |
|---|---|---|---|
| P1-01 | Aider-style repository map and task localization | In Progress | `GET /api/commander/repo-map`, `agentops commander repo-map`, and `scripts/commander_repo_map_smoke.py` provide deterministic file/symbol candidates with provenance and redacted snippets; remaining work is work-package artifact integration and merge-gate wiring |
