...
| P1-02 | Local Coding Project Template | In Progress | `GET /api/commander/coding-project-template`, `agentops commander coding-template`, `tpl_local_coding_project`, `scripts/commander_coding_project_template_smoke.py`, and `scripts/local_coding_project_template_smoke.py` now expose the read-only WorkPackage/worktree/patch/tests/verifier/merge-gate contract and create Commander packages with repo-map localization artifacts; next slice is executing the worktree/patch/verifier loop and writing those artifacts back to MIS |
