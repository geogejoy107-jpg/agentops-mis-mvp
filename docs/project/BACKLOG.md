...
| P1-05 | Split oversized horizontal modules | In Progress | Slices 1-3 extract runtime connector capability policy, registry row/upsert helpers, trust state read/update helpers, and read-model cache behavior into `agentops_mis_runtime/capabilities.py`, `agentops_mis_runtime/connectors.py`, `agentops_mis_runtime/trust.py`, and `agentops_mis_core/read_model_cache.py`; server keeps HTTP routes, health probing, refresh orchestration, runtime/audit writes, auth checks and read-model producers. Guarded by `scripts/module_boundary_smoke.py` plus runtime capability/readiness/trust and read-model cache smokes; continue with small strangler slices only |
