...
| P1-05 | Split oversized horizontal modules | In Progress | Slices 1-11 extract runtime connector capability policy, registry row/upsert helpers, refresh row projection, trust state read/update helpers, read-model cache behavior, Approval Wall prepared-action hash/public/readback, resume-gate mismatch, waiting-response and next-action helpers, worker fleet status/fleet-view aggregation, Commander work-package readback aggregation, Commander project-board gate aggregation, and Operator command-center gap/project/stale-ref/status aggregation into `agentops_mis_runtime/capabilities.py`, `agentops_mis_runtime/connectors.py`, `agentops_mis_runtime/trust.py`, `agentops_mis_core/read_model_cache.py`, `agentops_mis_core/approval_wall.py`, `agentops_mis_core/worker_fleet.py`, `agentops_mis_core/commander_work_packages.py`, and `agentops_mis_core/operator_command_center.py`; server keeps HTTP routes, health probing, SQLite upsert orchestration, runtime/audit writes, auth checks, SQLite producers, local daemon/process discovery, runtime adapter readiness, provider-specific prepared-action creation, Approval Wall exact-once resume writes, fleet hygiene mutations, and Commander planning/dispatch/synthesis writes. Guarded by `scripts/module_boundary_smoke.py` plus runtime capability/readiness/trust, read-model cache, prepared-action Approval Wall, runtime probe/Dify/Notion/customer-worker prepared-action gates, worker status/fleet, Commander project-board, Operator command-center, and Commander work-package smokes; continue with small strangler slices only |
