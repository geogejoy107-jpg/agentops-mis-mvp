...
| P1-05 | Split oversized horizontal modules | In Progress | Slices 1-7 extract runtime connector capability policy, registry row/upsert helpers, refresh row projection, trust state read/update helpers, read-model cache behavior, worker fleet status/fleet-view aggregation, Commander work-package readback aggregation, and Commander project-board gate aggregation into `agentops_mis_runtime/capabilities.py`, `agentops_mis_runtime/connectors.py`, `agentops_mis_runtime/trust.py`, `agentops_mis_core/read_model_cache.py`, `agentops_mis_core/worker_fleet.py`, and `agentops_mis_core/commander_work_packages.py`; server keeps HTTP routes, health probing, SQLite upsert orchestration, runtime/audit writes, auth checks, SQLite producers, local daemon/process discovery, runtime adapter readiness, fleet hygiene mutations, and Commander planning/dispatch/synthesis writes. Guarded by `scripts/module_boundary_smoke.py` plus runtime capability/readiness/trust, read-model cache, worker status/fleet, Commander project-board, and Commander work-package smokes; continue with small strangler slices only |
