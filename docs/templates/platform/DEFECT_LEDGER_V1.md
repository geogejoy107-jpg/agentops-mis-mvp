...
| C2-003 | P1 | same | The SDK mount test imports an uncommitted `sdk.py` from a mutable sibling C0 worktree; neither reviewed Git tree contains that object. | Mount against a frozen reviewed C0 commit/tree and record its exact hash. | Independent review failed |
| C2-004 | P1 | same | Migration SQL execution failure can commit partial `ALTER TABLE` changes while leaving the old schema version; only post-readback failure restores backup. | One transaction or authoritative backup restore for every execution exception; fault injection proves schema and version remain consistent. | Independent review failed |
| C2-005 | P1 | same | Manifest provenance points to intermediate `b7fefbee...`, not the exact reviewed implementation. | Governed packaging stamps and verifies the exact source commit; candidate documentation remains truthful before stamping. | Independent review failed |
| C2-006 | P2 | same | Scope test uses an intermediate base and handoff claims JSON Schema verification when the dependency was skipped. | Use frozen contract `1bce8f9e...` and run the schema test in the available `jsonschema` environment before updating handoff. | Independent review failed |
