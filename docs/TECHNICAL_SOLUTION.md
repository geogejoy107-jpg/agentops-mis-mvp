...
2. Vite/React 继续作为当前产品 UI；`ui/next-app` 已作为 Next.js 并行迁移轨道启动，先承接 workspace cockpit、agents、安全/worker readiness、tasks、runs、approvals、memory、audit、部署/BYOC 存储闸门与 `/api/mis/*` API proxy，再逐页通过 parity gate 替换。
