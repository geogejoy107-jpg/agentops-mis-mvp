...
- [x] Production-security warnings are prominent: `/workspace/agents` renders a top-level Production Security Boundary strip before operator controls, showing readiness, local write guard, auth/deployment status, startup security and the copyable next check command. Guarded by `scripts/security_production_readiness_smoke.py` and `scripts/operator_action_queue_ui_smoke.py`.
