...
- [x] Make panels independently loadable: AI Employees now uses `AI_EMPLOYEES_PANEL_LOADERS` and `AI_EMPLOYEES_SCOPED_PANEL_LOADERS` with `Promise.allSettled` plus `panelLoadState`, so one panel endpoint can become unavailable without failing the whole `/workspace/agents` page loader. `scripts/ai_employees_responsiveness_smoke.py` forbids the old monolithic destructured loader and requires the independent panel contract.
