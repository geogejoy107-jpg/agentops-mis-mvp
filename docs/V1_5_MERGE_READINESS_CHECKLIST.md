...
- [x] Make panels independently loadable: AI Employees now uses core/deferred `AI_EMPLOYEES_PANEL_LOADERS` plus `AI_EMPLOYEES_SCOPED_PANEL_LOADERS` with `Promise.allSettled` and `panelLoadState`, so core command-center data can render before secondary governance panels finish and one panel endpoint can become unavailable without failing the whole `/workspace/agents` page loader. Key panel headers render ready/unavailable/loading badges, and `scripts/ai_employees_responsiveness_smoke.py` forbids the old monolithic destructured loader while requiring the independent, visible panel contract.
