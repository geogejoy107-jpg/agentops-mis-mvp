...
def walk_items(value: Any, prefix: str = "") -> list[tuple[str, Any]]:
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            items.extend(walk_items(child, path))
...
def validate_manifest(data: dict[str, Any]) -> list[str]:
    ...
    for key, value in walk_items(data):
        lower = key.lower()
        if not any(fragment in lower for fragment in FORBIDDEN_KEY_FRAGMENTS):
            continue
        if lower == "governance.credentials_allowed" and value is False:
            continue
