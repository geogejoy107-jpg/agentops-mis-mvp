...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agentops-advance-loop-") as tmp:
        ...
        try:
            ...
            global_preview = run_cli(["operator", "advance-loop", "--fast-control", "--limit", "5"], base_url, outputs)
            ...
            require(global_preview_payload.get("control_source") in {None, "loop_control"}, f"global fast advance should use loop-control when confirmed: {global_preview_payload}", failures)
            require((global_preview_payload.get("preview") or {}).get("gate_id") == "runtime_doctor", f"global fast advance should prioritize runtime doctor: {global_preview_payload}", failures)
            require(((global_preview_payload.get("preview") or {}).get("action_policy") or {}).get("policy_id") == "advance_loop_local_bounded_v1", f"global runtime-doctor preview policy missing: {global_preview_payload}", failures)
            global_before_receipts = receipt_count_for_source(db_path, "loop_control:runtime_doctor")
            global_advanced = run_cli(["operator", "advance-loop", "--fast-control", "--limit", "5", "--confirm-advance"], base_url, outputs, timeout=45)
            global_advanced_payload = load_json(global_advanced.stdout)
            global_after_receipts = receipt_count_for_source(db_path, "loop_control:runtime_doctor")
            ...
            require(global_advanced_payload.get("control_source") == "loop_control", f"global advance should use fast loop-control: {global_advanced_payload}", failures)
            require((global_advanced_payload.get("preview") or {}).get("gate_id") == "runtime_doctor", f"global advance confirmed wrong gate: {global_advanced_payload}", failures)
            require(global_advanced_payload.get("advanced") is True, f"global advance should execute runtime doctor: {global_advanced_payload}", failures)
            require((global_advanced_payload.get("action_result") or {}).get("ok") is True, f"global runtime-doctor action failed: {global_advanced_payload}", failures)
            require((global_advanced_payload.get("verify_result") or {}).get("ok") is True, f"global runtime-doctor verify failed: {global_advanced_payload}", failures)
            ...
            require(global_before_control.get("selected_gate") == "runtime_doctor", f"global advance before control should target runtime doctor: {global_control}", failures)
            require(global_after_control.get("selected_gate") != "runtime_doctor", f"global advance after control should move past verified runtime doctor: {global_control}", failures)
            ...
            require(((global_readback_receipt.get("control_readback") or {}).get("before") or {}).get("selected_gate") == "runtime_doctor", f"global persisted control readback before mismatch: {global_advanced_payload}", failures)
            require(global_after_receipts >= global_before_receipts + 1, f"global runtime-doctor advance receipt missing: {global_before_receipts} -> {global_after_receipts}", failures)
            global_second_preview = run_cli(["operator", "advance-loop", "--fast-control", "--limit", "5"], base_url, outputs)
            ...
            require((global_second_payload.get("preview") or {}).get("gate_id") != "runtime_doctor", f"verified runtime-doctor work order should not be selected again: {global_second_payload}", failures)
            require((global_second_payload.get("preview") or {}).get("gate_id") == "handoff", f"global second preview should continue with handoff: {global_second_payload}", failures)
            require("agentops operator handoff" in str((global_second_payload.get("preview") or {}).get("action_command") or ""), f"handoff preview command missing: {global_second_payload}", failures)
            require(((global_second_payload.get("preview") or {}).get("action_policy") or {}).get("allowed") is True, f"handoff preview should be allowlisted: {global_second_payload}", failures)
            ...
            require(int(receipts_summary.get("control_readback_required") or 0) >= 1, f"action receipt summary should require control readbacks for advance receipts: {receipts_payload}", failures)
