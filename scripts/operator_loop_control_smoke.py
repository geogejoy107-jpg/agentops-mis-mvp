...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agentops-loop-control-") as tmp:
        ...
        try:
            ...
            handoff_confirm = run_cli(["operator", "advance-loop", "--fast-control", "--limit", "5", "--confirm-advance"], base_url, outputs, timeout=45)
            handoff_confirm_payload = load_json(handoff_confirm.stdout)
            require(handoff_confirm.returncode == 0, f"fast handoff confirm failed: {handoff_confirm.stderr or handoff_confirm.stdout}", failures)
            require((handoff_confirm_payload.get("receipt") or {}).get("status") == "verified", f"handoff receipt should verify: {handoff_confirm_payload}", failures)
            after_handoff_control = run_cli(["operator", "loop-control", "--limit", "5"], base_url, outputs)
            after_handoff_payload = load_json(after_handoff_control.stdout)
            after_handoff_selected = (((after_handoff_payload.get("work_order") or {}).get("advance_loop") or {}).get("selected_item") or {})
            require(after_handoff_selected.get("gate_id") == "action_plan", f"verified handoff receipt should advance loop-control to action_plan: {after_handoff_payload}", failures)
            require("operator action-plan" in (after_handoff_selected.get("action_command") or ""), f"loop-control action-plan command missing after verified handoff: {after_handoff_selected}", failures)
            action_plan_confirm = run_cli(["operator", "advance-loop", "--fast-control", "--limit", "5", "--confirm-advance"], base_url, outputs, timeout=45)
            action_plan_confirm_payload = load_json(action_plan_confirm.stdout)
            require(action_plan_confirm.returncode == 0, f"fast action-plan confirm failed: {action_plan_confirm.stderr or action_plan_confirm.stdout}", failures)
            require((action_plan_confirm_payload.get("receipt") or {}).get("status") == "verified", f"action-plan receipt should verify: {action_plan_confirm_payload}", failures)
            after_action_plan_control = run_cli(["operator", "loop-control", "--limit", "5"], base_url, outputs)
            after_action_plan_payload = load_json(after_action_plan_control.stdout)
            after_action_plan_selected = (((after_action_plan_payload.get("work_order") or {}).get("advance_loop") or {}).get("selected_item") or {})
            require(after_action_plan_selected.get("gate_id") == "review_queue", f"verified action-plan receipt should advance loop-control to review_queue when reviews exist: {after_action_plan_payload}", failures)
            require("review queue" in (after_action_plan_selected.get("action_command") or ""), f"loop-control review queue command missing after verified action-plan: {after_action_plan_selected}", failures)
            review_queue_confirm = run_cli(["operator", "advance-loop", "--fast-control", "--limit", "5", "--confirm-advance"], base_url, outputs, timeout=45)
            review_queue_confirm_payload = load_json(review_queue_confirm.stdout)
            require(review_queue_confirm.returncode == 0, f"fast review-queue confirm failed: {review_queue_confirm.stderr or review_queue_confirm.stdout}", failures)
            require((review_queue_confirm_payload.get("receipt") or {}).get("status") == "verified", f"review-queue receipt should verify: {review_queue_confirm_payload}", failures)
            after_review_queue_control = run_cli(["operator", "loop-control", "--limit", "5"], base_url, outputs)
            after_review_queue_payload = load_json(after_review_queue_control.stdout)
            after_review_queue_selected = (((after_review_queue_payload.get("work_order") or {}).get("advance_loop") or {}).get("selected_item") or {})
            require(after_review_queue_payload.get("status") == "ready", f"verified review-queue receipt should make unscoped loop-control ready: {after_review_queue_payload}", failures)
            require(not after_review_queue_selected, f"ready loop-control should not keep selecting a completed ladder item: {after_review_queue_selected}", failures)
