...
def smoke() -> dict:
    ...
    sensitive = "email joy@example.com phone +1 (415) 555-0123 Authorization: Bearer sk-demo-secret token=ntn_demo_secret raw ntn_raw_secret"
    ...
    worker_sensitive = worker_redact_text("Authorization: Bearer sk-worker-secret token=abc123", 200)
    require("sk-worker-secret" not in worker_sensitive, f"worker secret leaked: {worker_sensitive}")

    return {
        "ok": True,
        "safe_preserved": ["127.0.0.1:8642", "tsk_worker_hermes_live_20260618065503", "run_gw_6f995c9de929"],
        "sensitive_redacted": ["email", "phone", "bearer", "token"],
    }
