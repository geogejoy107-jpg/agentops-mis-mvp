...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agentops-loop-driver-") as tmp:
        ...
        try:
            ...
            preview_current_code_gate = (((initial_brief.get("local_run_path") or {}).get("current_code_gate") or {}))
            preview_read_current_code = next((gate for gate in preview_agent_method_gates if gate.get("id") == "read_current_code"), {})
            ...
            require(preview_acceptance_decision.get("current_code_ok") is True, f"preview acceptance current-code proof missing: {preview_acceptance}", failures)
            ...
            require((preview_agent_packet.get("gates") or {}).get("current_code") == "ready", f"preview agent current-code gate should be ready: {preview_agent_packet}", failures)
            require(preview_read_current_code.get("status") == "ready", f"preview read_current_code gate should be ready: {preview_agent_packet}", failures)
            require(preview_current_code_gate.get("ok") is True and preview_current_code_gate.get("status") == "current", f"preview launch brief current-code proof missing: {initial_brief}", failures)
            require("--expect-head-sha" in str(preview_current_code_gate.get("strict_command") or ""), f"preview strict current-code command missing expected head: {preview_current_code_gate}", failures)
            ...
            require({"read_start_check", "read_current_code", "plan_agent_plan", "retrieve_knowledge", "compare_base_reference", "preflight_adapter", "execute_bounded_loop", "verify_loop", "record_memory_candidate"}.issubset(preview_agent_method_gate_ids), f"preview agent method gates missing: {preview_agent_packet}", failures)
            ...
            require((final_agent_packet.get("gates") or {}).get("current_code") == "ready", f"confirm final current-code gate should be ready: {final_agent_packet}", failures)
            ...
            final_read_current_code = next((gate for gate in (final_agent_packet.get("method_gates") or []) if gate.get("id") == "read_current_code"), {})
            require(final_read_current_code.get("status") == "ready", f"confirm final read_current_code gate should be ready: {final_agent_packet}", failures)
            require({"read_current_code", "plan_agent_plan", "retrieve_knowledge", "compare_base_reference", "record_memory_candidate"}.issubset({gate.get("id") for gate in (final_agent_packet.get("method_gates") or [])}), f"confirm final agent method gates missing: {final_agent_packet}", failures)
            ...
            final_current_code_gate = (((final_brief.get("local_run_path") or {}).get("current_code_gate") or {}))
            require(final_current_code_gate.get("ok") is True and final_current_code_gate.get("status") == "current", f"confirm final launch brief current-code proof missing: {final_brief}", failures)
