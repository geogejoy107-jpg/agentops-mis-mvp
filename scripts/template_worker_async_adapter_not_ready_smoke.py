#!/usr/bin/env python3
"""Verify async template worker submit is rejected before queueing when a live adapter is unavailable."""
...
def main() -> int:
    ...
    try:
        ...
        with server.db() as conn:
            ...
            payload, status = server.submit_customer_task_template_job(conn, {
                "template_id": "tpl_customer_ui_review",
                "adapter": "openclaw",
                "confirm_run": True,
                "title": "Async template adapter not ready smoke",
                "description": "This template async submit must reject before queueing because OpenClaw is unavailable.",
            })
            ...
            rejected_events = conn.execute(
                "SELECT COUNT(*) c FROM runtime_events WHERE event_type='workflow_job.rejected' AND task_id=?",
                (task_id,),
            ).fetchone()["c"]
        require(status == 409, f"expected 409, got {status}: {payload}")
        require(payload.get("ok") is False, f"async template reject should not be ok: {payload}")
        ...
        require((result.get("template_execution") or {}).get("adapter") == "openclaw", f"template execution metadata missing: {payload}")
        require(rejected_events >= 1, "workflow_job.rejected runtime event missing")
        serialized = json.dumps(payload, ensure_ascii=False)
        require(not leaked_secret(serialized), "async template adapter-not-ready payload leaked token-like material")
        print(json.dumps({
            "ok": True,
            "status": status,
