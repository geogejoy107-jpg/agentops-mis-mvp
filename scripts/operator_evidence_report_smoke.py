...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agentops-evidence-report-") as tmp:
        ...
        try:
            ...
            missing_memory_commands = " ".join(missing_memory_item.get("recommended_commands") or [])
            ...
            require(missing_memory_item.get("execution_evidence_gap") is False, f"memory-only attention should not be an execution evidence gap: {missing_memory_item}", failures)
            ...
            require("memory propose" in missing_memory_commands, f"missing memory command absent: {missing_memory_item}", failures)
            require("remediate-evidence-gap" not in missing_memory_commands, f"memory-only attention should not recommend remediation task: {missing_memory_item}", failures)
            ...
            pending_memory_commands = " ".join(pending_memory_item.get("recommended_commands") or [])
            ...
            require(pending_memory_item.get("execution_evidence_gap") is False, f"pending memory review should not be an execution evidence gap: {pending_memory_item}", failures)
            ...
            require("memory list" in pending_memory_commands, f"pending memory command absent: {pending_memory_item}", failures)
            require("remediate-evidence-gap" not in pending_memory_commands, f"pending memory review should not recommend remediation task: {pending_memory_item}", failures)
            pending_handoff = run_cli(["operator", "handoff", "--limit", "12"], base_url, agent_id, outputs)
            pending_remediation_items = (((pending_handoff.get("work_order") or {}).get("evidence_report") or {}).get("remediation_chain") or {}).get("items") or []
            require(not any(item.get("run_id") == run_id for item in pending_remediation_items), f"memory-only attention leaked into remediation chain: {pending_handoff}", failures)
            ...
            require(item.get("execution_evidence_gap") is False, f"ready run should not expose execution evidence gap: {item}", failures)
