...
def validate_timeout_endpoint(payload: dict, failures: list[str]) -> None:
    require(payload.get("operation") == "operator_loop_bootstrap", f"timeout operation mismatch: {payload}", failures)
    require(payload.get("status") == "blocked", f"timeout should block: {payload}", failures)
    require(payload.get("error_type") == "local_mis_endpoint_timeout", f"timeout error type missing: {payload}", failures)
    require(payload.get("missing_endpoint") == "/api/operator/start-check", f"timeout endpoint mismatch: {payload}", failures)
    diagnostic = payload.get("diagnostic") or {}
    require("retry_with_longer_timeout_command" in diagnostic, f"timeout retry command missing: {payload}", failures)
    require("--request-timeout 120" in str(diagnostic.get("retry_with_longer_timeout_command")), f"timeout retry should raise request timeout: {payload}", failures)
    safety = payload.get("safety") or {}
    require(safety.get("read_only") is True, f"timeout read-only proof missing: {payload}", failures)
    require(safety.get("ledger_mutated") is False, f"timeout ledger proof missing: {payload}", failures)
    require(safety.get("server_executes_shell") is False, f"timeout shell proof missing: {payload}", failures)
    require(safety.get("live_execution_performed") is False, f"timeout live proof missing: {payload}", failures)
    require(payload.get("token_omitted") is True, f"timeout token proof missing: {payload}", failures)
...
def main() -> int:
    ...
    try:
        ...
        validate_timeout_endpoint(timeout_payload, failures)
    finally:
