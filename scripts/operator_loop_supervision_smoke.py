...
def validate(payload: dict, failures: list[str], *, expect_quality_attention: bool = False, expect_service_primary: bool = False) -> None:
    ...
    require(summary.get("research_lab_consumptions") == 2, f"top-level research lab consumption summary missing: {summary}", failures)
    require(summary.get("research_lab_consumed") == 0, f"isolated loop should start with no Research Lab consumption receipts: {summary}", failures)
    require(summary.get("research_lab_consumption_missing") == 2, f"isolated loop should surface missing Research Lab consumption receipts: {summary}", failures)
    ...
    for item in items:
        ...
        research_lab_consumption = item.get("research_lab_consumption") or {}
        require(research_lab_consumption.get("operation") == "operator_research_lab_consumption_status", f"{adapter} Research Lab consumption readback missing: {research_lab_consumption}", failures)
        require(research_lab_consumption.get("status") == "missing", f"{adapter} Research Lab consumption should start missing: {research_lab_consumption}", failures)
        require(research_lab_consumption.get("consumed") is False, f"{adapter} Research Lab consumption should not be consumed yet: {research_lab_consumption}", failures)
        require(research_lab_consumption.get("packet_hash") == research_lab_packet.get("packet_hash"), f"{adapter} Research Lab consumption hash mismatch: {research_lab_consumption}", failures)
        require("agentops operator research-lab-consumption" in str(research_lab_consumption.get("record_command") or ""), f"{adapter} Research Lab consumption record command missing: {research_lab_consumption}", failures)
        require("--confirm-record" in str(research_lab_consumption.get("record_command") or ""), f"{adapter} Research Lab consumption record must be confirmation-gated: {research_lab_consumption}", failures)
        consumption_safety = research_lab_consumption.get("safety") or {}
        require(consumption_safety.get("read_only") is True, f"{adapter} Research Lab consumption read-only proof missing: {consumption_safety}", failures)
        require(consumption_safety.get("server_executes_shell") is False, f"{adapter} Research Lab consumption shell proof missing: {consumption_safety}", failures)
        research_lab_consumption_contract = evidence_contract.get("research_lab_consumption") or {}
        require(evidence_contract.get("research_lab_consumption_required") is True, f"{adapter} Research Lab consumption contract flag missing: {evidence_contract}", failures)
        require(research_lab_consumption_contract.get("status") == "missing", f"{adapter} Research Lab consumption contract status wrong: {research_lab_consumption_contract}", failures)
        require(research_lab_consumption_contract.get("consumed") is False, f"{adapter} Research Lab consumption contract should be unconsumed: {research_lab_consumption_contract}", failures)
        require(research_lab_consumption_contract.get("packet_hash") == research_lab_packet.get("packet_hash"), f"{adapter} Research Lab consumption contract hash mismatch: {research_lab_consumption_contract}", failures)
        require("--confirm-record" in str(research_lab_consumption_contract.get("record_command") or ""), f"{adapter} Research Lab consumption contract record command missing: {research_lab_consumption_contract}", failures)
        require(research_lab_consumption_contract.get("hard_run_start_gate") is False, f"{adapter} Research Lab consumption should not be hard run_start gate: {research_lab_consumption_contract}", failures)
        require(research_lab_consumption_contract.get("server_executes_shell") is False, f"{adapter} Research Lab consumption contract shell proof missing: {research_lab_consumption_contract}", failures)
        ...
        research_lab_consumption_gate = next((gate for gate in (item.get("gates") or []) if gate.get("id") == "research_lab_consumption"), {})
        require(research_lab_consumption_gate.get("status") == "attention", f"{adapter} Research Lab consumption gate should be attention before record: {research_lab_consumption_gate}", failures)
        require(research_lab_consumption_gate.get("ok") is False, f"{adapter} Research Lab consumption gate should not pass before record: {research_lab_consumption_gate}", failures)
        require(research_lab_consumption_gate.get("packet_hash") == research_lab_packet.get("packet_hash"), f"{adapter} Research Lab consumption gate hash mismatch: {research_lab_consumption_gate}", failures)
        require("--confirm-record" in str(research_lab_consumption_gate.get("command") or ""), f"{adapter} Research Lab consumption gate command missing: {research_lab_consumption_gate}", failures)
        require(research_lab_consumption_gate.get("hard_run_start_gate") is False, f"{adapter} Research Lab consumption gate should not hard-block run_start: {research_lab_consumption_gate}", failures)
        require(research_lab_consumption_gate.get("server_executes_shell") is False, f"{adapter} Research Lab consumption gate shell proof missing: {research_lab_consumption_gate}", failures)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agentops-loop-supervision-") as tmp:
        ...
        try:
            ...
            require(http_packet_summary.get("research_lab_packets") == 2, f"HTTP work packet Research Lab summary missing: {http_packet_summary}", failures)
            ...
            packet_http_summary = http_packet_payload.get("summary") or {}
            packet_cli_summary = packet_payload.get("summary") or {}
            for key in [
                "items",
                "ready_to_confirm",
                "record_first",
                ...
                "research_lab_consumed",
                "research_lab_consumption_missing",
                "can_confirm_all",
                "record_required",
                "current_code_ok",
                "work_packets",
            ]:
                require(packet_cli_summary.get(key) == packet_http_summary.get(key), f"CLI/HTTP work packet summary drift for {key}: cli={packet_cli_summary} http={packet_http_summary}", failures)
