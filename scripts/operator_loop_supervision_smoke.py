...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agentops-loop-supervision-") as tmp:
        ...
        try:
            ...
            http_by_adapter = {item.get("adapter"): item for item in http_decisions if item.get("adapter")}
            cli_by_adapter = {item.get("adapter"): item for item in decisions if item.get("adapter")}
            require(set(cli_by_adapter) == set(http_by_adapter), f"CLI/HTTP decision adapter drift: cli={cli_by_adapter} http={http_by_adapter}", failures)
            for adapter, cli_item in cli_by_adapter.items():
                http_item = http_by_adapter.get(adapter) or {}
                for field in ["decision", "phase", "command", "confirm_required"]:
                    require(cli_item.get(field) == http_item.get(field), f"CLI/HTTP decision drift for {adapter}.{field}: cli={cli_item} http={http_item}", failures)
                require((cli_item.get("policy") or {}).get("server_may_execute") == (http_item.get("policy") or {}).get("server_may_execute"), f"CLI/HTTP policy drift for {adapter}: cli={cli_item} http={http_item}", failures)
