...
def main() -> int:
    ...
    require((report.get("report_boundary") or {}).get("customer_markdown_excludes_internal_evidence") is True, f"report boundary missing customer exclusion proof: {report}", failures)
    require((report.get("report_boundary") or {}).get("internal_evidence_separated") is True, f"report boundary missing internal evidence separation: {report}", failures)
    require((report.get("internal_evidence") or {}).get("visibility") == "internal_operator", f"internal evidence visibility missing: {report}", failures)
    require(((report.get("internal_evidence") or {}).get("counts") or {}).get("tool_calls", 0) >= 1, f"internal evidence should retain operator tool-call counts: {report}", failures)
    require((report.get("safe_defaults") or {}).get("raw_prompts_stored") is False, f"safe defaults missing raw prompt omission: {report}", failures)
    require((report.get("safe_defaults") or {}).get("raw_model_responses_stored") is False, f"safe defaults missing raw response omission: {report}", failures)
    ...
    require("Raw prompts, raw model responses and private transcripts included in this customer report: false" in markdown, "markdown missing internal evidence exclusion boundary", failures)
    require("Delivery Progress" in markdown, "markdown missing delivery progress section", failures)
    require("Task Ledger" not in markdown, "customer markdown should not expose internal task ledger section", failures)
    require("Tool calls:" not in markdown and "Memory candidates:" not in markdown, "customer markdown should not expose internal evidence counters", failures)
