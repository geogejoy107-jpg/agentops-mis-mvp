...
def create_verified_plan(base_url: str, agent_id: str, task_id: str) -> str:
    status, plan = http_json("POST", base_url, "/api/agent-gateway/agent-plans", {
        ...
        "agent_id": agent_id,
        "task_id": task_id,
        "task_understanding": "Start the fleet hygiene fixture only after a verified plan is recorded.",
        ...
        "proposed_files_to_change": ["scripts/worker_fleet_hygiene_smoke.py"],
        "risk_level": "low",
        "execution_steps": ["READ", "PLAN", "RETRIEVE", "VERIFY"],
        "verification_plan": "Run worker_fleet_hygiene_smoke.py.",
