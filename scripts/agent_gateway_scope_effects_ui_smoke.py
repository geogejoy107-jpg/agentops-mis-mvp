...
def main() -> int:
    ...
    worker_scope_readiness = extract_block(ai, 'data-testid="agent-gateway-worker-scope-readiness"', "enrollmentPolicy.invalid_scopes")
    ...
    require("WORKER_EXECUTION_REQUIRED_SCOPES" in ai, "AI Employees page missing worker execution required-scope model", failures)
    require("scopeEffectRows" in ai, "AI Employees page missing scope effect row model", failures)
    require("missingSelectedWorkerScopes" in ai and "selectedScopeWorkerViable" in ai, "AI Employees page missing worker viability calculation", failures)
    ...
    require("agent-gateway-worker-scope-readiness" in ai, "AI Employees page missing worker-scope readiness panel", failures)
    require("workerViability" in scope_panel and "requiredWorkerScopes" in worker_scope_readiness, "worker viability copy missing from scope panel", failures)
    require("tasks:read" in ai and "tasks:claim" in ai and "evaluations:submit" in ai, "worker required scopes do not cover pull/claim/evaluation loop", failures)
    require("HTTP 403" in ai and "服务端执行这些 endpoint scope" in ai, "English/Chinese scope enforcement copy missing", failures)
    require("Worker 可执行性" in ai and "缺少 worker-loop scope" in ai, "Chinese worker viability copy missing", failures)
