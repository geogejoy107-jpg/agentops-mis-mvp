    ...
    try:
        with tempfile.TemporaryDirectory(prefix="agentops-next-worker-console-") as tmp:
            ...
            snapshot = snapshot_route(next_base, "/workspace/workers", [
                ...
                "Worker Console coverage boundary",
                "Agent Gateway CLI/API/MCP canonical for token issue/rotate/revoke",
                "Agent Gateway CLI/API/MCP canonical for session lifecycle",
                "live daemon lifecycle requires CLI/API operator lane",
                "Vite route retirement blocked",
