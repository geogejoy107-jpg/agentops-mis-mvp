...
def cmd_operator_start_check(args, client: AgentOpsClient) -> dict:
    adapter = args.adapter
    local = client.get("/api/local/readiness")
    worker_readiness = client.get("/api/workers/adapter-readiness")
    runtime_doctor = cmd_operator_runtime_doctor(
        argparse.Namespace(loop_id=args.loop_id, limit=args.limit, runtime_base_url=args.runtime_base_url),
        client,
    )
    launch_brief = cmd_operator_loop_launch_packet(
        argparse.Namespace(
            limit=args.limit,
            task_id=args.task_id,
            agent_id=args.agent_id,
            query=args.query,
            handoff_mode=args.handoff_mode,
            full_handoff=args.full_handoff,
            brief=True,
            adapter=adapter,
        ),
        client,
    ...
    if adapter in {"hermes", "openclaw"}:
        live_product = cmd_operator_live_product_readiness(
            argparse.Namespace(freshness_hours=args.freshness_hours, limit=args.limit, require_adapter=[adapter]),
            client,
        )
        if isinstance(live_product, dict):
            live_product = {key: value for key, value in live_product.items() if key != "_exit_code"}
    ...
    adapter_readiness = str(adapter_item.get("readiness") or "unknown")
    local_summary = local.get("summary") if isinstance(local.get("summary"), dict) else {}
    local_run_path = launch_brief.get("local_run_path") if isinstance(launch_brief.get("local_run_path"), dict) else {}
    ...
    if not worker_policy:
        ...
        if "server_executes_shell" not in worker_policy:
            worker_policy["server_executes_shell"] = worker_policy_safety.get("server_executes_shell", False)
        if "token_omitted" not in worker_policy:
            worker_policy["token_omitted"] = bool(
                worker_policy_safety.get("token_omitted") is True
                or (worker_policy.get("session") or {}).get("token_omitted") is True
            )
    worker_policy_safety = worker_policy.get("safety") if isinstance(worker_policy.get("safety"), dict) else {}
    worker_policy_server_shell = (
        worker_policy.get("server_executes_shell")
        if "server_executes_shell" in worker_policy
        else worker_policy_safety.get("server_executes_shell")
    )
    worker_policy_token_omitted = (
        worker_policy.get("token_omitted")
        if "token_omitted" in worker_policy
        else worker_policy_safety.get("token_omitted", True)
    )
    ...
    gates = [
        ...
        _start_check_gate(
            ...
            command=f"agentops operator loop-launch-packet --brief --adapter {adapter} --limit {args.limit}",
        ),
        _start_check_gate(
            "local_run_path",
        ...
        _start_check_gate(
            ...
            ok=bool((launch_brief.get("summary") or {}).get("agent_plan_risk")) and launch_brief.get("safety", {}).get("token_omitted") is True,
            detail=(
                f"risk={(launch_brief.get('summary') or {}).get('agent_plan_risk')}; "
                f"approval_required={(launch_brief.get('summary') or {}).get('agent_plan_approval_required')}"
            ),
        ...
        _start_check_gate(
            ...
            command=(
                ...
                if adapter == "mock"
                else f"agentops operator live-product-readiness --require-adapter {adapter}"
            ),
        ),
    ]
    ...
    next_commands = [
        ...
        f"agentops operator loop-launch-packet --brief --adapter {adapter} --limit {args.limit}",
    ...
    return {
        ...
        "summary": {
            ...
            "control_status": (launch_brief.get("summary") or {}).get("control_status"),
            "recommended_step": (launch_brief.get("summary") or {}).get("recommended_step"),
        ...
        "launch_brief": {
            "operation": launch_brief.get("operation"),
            "status": launch_brief.get("status"),
            "summary": launch_brief.get("summary"),
            ...
            "adapter_preflight_command": launch_brief.get("adapter_preflight_command"),
            "live_run_command": launch_brief.get("live_run_command"),
            "readback_commands": launch_brief.get("readback_commands") or [],
            "policy": launch_brief.get("policy"),
            "safety": launch_brief.get("safety"),
            "token_omitted": True,
        },
