...
async function taskClaimBody(request: Request) {
  return boundedJsonObject(request, {
    maxBytes: TASK_CLAIM_MAX_BODY_BYTES,
    allowEmpty: true,
    label: "Task claim",
  });
}
...
export async function getAgentGatewayTask(
  request: Request,
  ...
  return withPostgresTransaction(async (client) => {
    ...
    if (!agentCanAccessTask(task, identity.agentId)) {
      throw new ControlPlaneHttpError(
        403,
        "forbidden",
...
export async function pullAgentGatewayTasks(request: Request) {
  ...
  return withPostgresTransaction(async (client) => {
    ...
    const requestedTaskId = suppliedBinding(url.searchParams.get("task_id"));
    const filteredTaskId = requestedTaskId
      ? requiredTaskId(requestedTaskId)
      : null;
    const result = await client.query<TaskRow>(
      `SELECT * FROM tasks
