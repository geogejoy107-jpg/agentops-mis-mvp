...
async function bodyObject(request: Request) {
  return boundedJsonObject(request, {
    maxBytes: AGENT_PLAN_MAX_BODY_BYTES,
    label: "Agent Plan",
  });
}

function planVersion(value: unknown, fallback = 1) {
  ...
  if (!Number.isSafeInteger(parsed) || parsed < 1 || parsed > 1_000_000) {
    throw new ControlPlaneHttpError(
      ...
      "plan_version must be an integer from 1 through 1000000.",
    );
  }
  return parsed;
}
...
export async function createAgentGatewayPlan(request: Request) {
  ...
  return withPostgresTransaction(async (client) => {
    ...
    const row: AgentPlanRow = {
      ...
      plan_version: planVersion(body.plan_version, existing?.plan_version || 1),
      plan_hash: null,
      verified_at: existing?.verified_at || null,
      verification_result_hash: existing?.verification_result_hash || null,
      created_at: existing?.created_at || now,
      updated_at: existing?.updated_at || now,
    };
    row.plan_hash = computeAgentPlanHash(row);
    if (existing?.plan_hash && existing.plan_hash !== row.plan_hash) {
      throw new ControlPlaneHttpError(409, "agent_plan_hash_mismatch", "Agent Plan content no longer matches its stored plan_hash.");
    }
    ...
    if (existing) {
      await client.query(
        `UPDATE agent_plans SET task_understanding=$1,referenced_specs_json=$2,referenced_memories_json=$3,
          ...
          execution_steps_json=$8,verification_plan=$9,rollback_plan=$10,status=$11,plan_version=$12,
          plan_hash=$13,verified_at=NULL,verification_result_hash=NULL,updated_at=$14 WHERE plan_id=$15`,
        [row.task_understanding, row.referenced_specs_json, row.referenced_memories_json, row.referenced_bases_json,
          ...
          row.verification_plan, row.rollback_plan, row.status, row.plan_version, row.plan_hash, row.updated_at, row.plan_id],
      );
    } else {
      await client.query(
        `INSERT INTO agent_plans(plan_id,workspace_id,task_id,run_id,agent_id,task_understanding,referenced_specs_json,
          ...
          execution_steps_json,verification_plan,rollback_plan,status,plan_version,plan_hash,verified_at,
          verification_result_hash,created_at,updated_at)
        ...
        [row.plan_id, row.workspace_id, row.task_id, row.run_id, row.agent_id, row.task_understanding,
          ...
          row.proposed_files_to_change_json, row.risk_level, row.approval_required, row.execution_steps_json,
          row.verification_plan, row.rollback_plan, row.status, row.plan_version, row.plan_hash, row.verified_at,
          row.verification_result_hash, row.created_at, row.updated_at],
      );
    }
...
function planVerification(row: AgentPlanRow) {
  const computedHash = computeAgentPlanHash(row);
  const verification = verifyAgentPlanRow({ ...row, plan_hash: computedHash });
  if (row.plan_hash && row.plan_hash !== computedHash) {
    const hashCheck = {
      id: "plan_hash_match",
      ok: false,
      message: "Stored immutable Plan hash matches current Plan content.",
    };
    return {
      ...verification,
      pass: false,
      plan_hash: computedHash,
      checks: [...verification.checks, hashCheck],
      failed_checks: [...verification.failed_checks, hashCheck],
  ...
  return {
    ...
    stored_plan_hash: row.plan_hash || computedHash,
  };
}

export async function listAgentGatewayPlans(request: Request) {
  return withPostgresTransaction(async (client) => {
    const identity = await authenticateAgentGateway(client, request.headers, "agent_plans:read");
    ...
    if (requestedAgent && requestedAgent !== identity.agentId) {
      throw new ControlPlaneHttpError(403, "forbidden", "Agent credential cannot list another agent's plans.");
    }
    ...
    const runId = optionalIdentifier(url.searchParams.get("run_id"), "run_id");
    const where = ["workspace_id=$1", "agent_id=$2"];
    const params: unknown[] = [identity.workspaceId, identity.agentId];
    if (taskId) {
      params.push(taskId);
    ...
    params.push(limit);
    const result = await client.query<AgentPlanRow>(
      `SELECT * FROM agent_plans WHERE ${where.join(" AND ")}
      ORDER BY created_at DESC,plan_id DESC LIMIT $${params.length}`,
      params,
    );
    return {
      provider: "agentops-agent-plan",
...
export async function getAgentGatewayPlan(request: Request, planIdValue: string, verify = false) {
  const planId = identifier(planIdValue, "plan_id");
  return withPostgresTransaction(async (client) => {
    const identity = await authenticateAgentGateway(client, request.headers, "agent_plans:read");
    enforceWorkspaceBinding(identity, {
      header: request.headers.get("x-agentops-workspace-id"),
    ...
    const result = await client.query<AgentPlanRow>(
      "SELECT * FROM agent_plans WHERE plan_id=$1 AND workspace_id=$2 AND agent_id=$3 FOR UPDATE",
      [planId, identity.workspaceId, identity.agentId],
    );
    const row = result.rows[0];
    if (!row) throw new ControlPlaneHttpError(404, "agent_plan_not_found", "Agent Plan was not found.");
    if (!verify) {
      return {
        provider: "agentops-agent-plan",
        control_plane: "typescript_postgres",
        operation: "agent_plan_get",
        agent_plan: row,
        token_omitted: true,
      };
    }
    const verification = planVerification(row);
    const verificationResultHash = agentPlanVerificationHash(
      row.plan_id,
      verification,
    );
    const unchanged = row.verification_result_hash === verificationResultHash
      && Boolean(row.verified_at);
    const verifiedAt = unchanged
      ? String(row.verified_at)
      : new Date().toISOString();
    if (!unchanged) {
      await client.query(
        `UPDATE agent_plans SET verified_at=$1,verification_result_hash=$2,updated_at=$1
        WHERE plan_id=$3 AND workspace_id=$4 AND agent_id=$5`,
        [verifiedAt, verificationResultHash, planId, identity.workspaceId, identity.agentId],
      );
    }
    return {
      provider: "agentops-agent-plan",
...
async function verifyManifest(
  ...
  const checks = [
    { id: "plan_hash_bound", ok: Boolean(manifest.plan_hash), message: "Manifest binds an immutable Plan hash." },
    { id: "plan_hash_match", ok: manifest.plan_hash === plan.plan_hash && manifest.plan_hash === currentPlanHash, message: "Manifest Plan hash matches stored and current Plan content." },
    ...
    { id: "plan_verified_at", ok: Number.isFinite(Date.parse(String(plan.verified_at || ""))), message: "Referenced agent_plan has a persisted verification timestamp." },
    { id: "plan_verification_hash", ok: plan.verification_result_hash === agentPlanVerificationHash(plan.plan_id, planVerification), message: "Referenced agent_plan verification hash matches current canonical verification." },
    ...
    { id: "run_plan_id_match", ok: run.agent_plan_id === plan.plan_id, message: "Run pins the referenced Agent Plan id." },
    { id: "run_plan_hash_match", ok: run.plan_hash === plan.plan_hash, message: "Run pins the current Agent Plan hash." },
    { id: "manifest_verification_hash_match", ok: manifest.verification_result_hash === plan.verification_result_hash, message: "Manifest pins the current Agent Plan verification hash." },
...
export async function verifyLatestWorkspacePlanEvidence(
  ...
  const planResult = await client.query<AgentPlanRow>(
    ...
    verification_plan,rollback_plan,status,plan_version,plan_hash,verified_at,verification_result_hash,created_at,updated_at
...
function sameManifestBinding(left: ManifestRow, right: ManifestRow) {
  const fields: Array<keyof ManifestRow> = [
    ...
    "tool_call_ids_json", "evaluation_ids_json", "artifact_ids_json", "audit_ids_json", "plan_hash",
...
export async function createAgentGatewayPlanEvidenceManifest(request: Request) {
  ...
  return withPostgresTransaction(async (client) => {
    ...
    const runCandidateResult = await client.query<
      Pick<RunRow, "task_id" | "agent_id" | "agent_plan_id" | "plan_hash">
    >(
      `SELECT task_id,agent_id,agent_plan_id,plan_hash
      FROM runs WHERE run_id=$1 AND workspace_id=$2`,
    ...
    if (runCandidate.agent_plan_id !== planId) {
      throw new ControlPlaneHttpError(
        409,
        "manifest_run_plan_binding_conflict",
        "Manifest run is not bound to the referenced Agent Plan.",
      );
    }
    ...
    if (
      ...
      throw new ControlPlaneHttpError(
        403,
        "forbidden",
        "Manifest task is no longer assigned to the credential agent.",
      );
    }
    ...
    const currentPlanHash = computeAgentPlanHash(plan);
    if (!plan.plan_hash || plan.plan_hash !== currentPlanHash) {
      throw new ControlPlaneHttpError(
        409,
    ...
    const currentVerification = verifyAgentPlanRow(plan);
    if (
      !plan.verified_at
      || !plan.verification_result_hash
      || plan.verification_result_hash
        !== agentPlanVerificationHash(plan.plan_id, currentVerification)
    ) {
      throw new ControlPlaneHttpError(
        409,
        "agent_plan_current_verification_required",
        "Manifest requires a current persisted Agent Plan verification.",
      );
    }
    if (
      run.agent_plan_id !== plan.plan_id
      || run.plan_hash !== plan.plan_hash
      || runCandidate.plan_hash !== plan.plan_hash
    ) {
      throw new ControlPlaneHttpError(
        409,
        "manifest_run_plan_binding_conflict",
        "Run no longer binds the current Agent Plan id and hash.",
      );
    }
    if (
      ...
      throw new ControlPlaneHttpError(
        ...
        "Supplied plan_hash does not match the current Agent Plan.",
      );
    }
    if (
      body.verification_result_hash !== undefined
      && String(body.verification_result_hash) !== plan.verification_result_hash
    ) {
      throw new ControlPlaneHttpError(
        409,
        "manifest_stale_verification_hash",
        "Supplied verification_result_hash does not match current verification.",
      );
    }
    ...
    if (verification) {
      ...
      candidate.verification_result_hash = plan.verification_result_hash;
    }
    ...
    if (existing) {
      await client.query(
        ...
        SET status=$1,verification_json=$2,verification_result_hash=$3,updated_at=$4
        WHERE manifest_id=$5`,
        [candidate.status, candidate.verification_json, candidate.verification_result_hash, candidate.updated_at, manifestId],
      );
    } else {
      await client.query(
        `INSERT INTO plan_evidence_manifests(manifest_id,workspace_id,plan_id,task_id,run_id,agent_id,mismatch_policy,
          expected_steps_json,tool_call_ids_json,evaluation_ids_json,artifact_ids_json,audit_ids_json,plan_hash,
        ...
        [candidate.manifest_id, candidate.workspace_id, candidate.plan_id, candidate.task_id, candidate.run_id,
          ...
          candidate.evaluation_ids_json, candidate.artifact_ids_json, candidate.audit_ids_json, candidate.plan_hash,
          candidate.verification_result_hash, candidate.status, candidate.verification_json, candidate.created_at,
