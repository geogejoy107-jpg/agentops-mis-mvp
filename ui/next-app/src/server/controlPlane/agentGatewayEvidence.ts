...
export async function recordAgentGatewayArtifact(request: Request) {
  ...
  return withPostgresTransaction(async (client) => {
    ...
    const contentHash = body.content_hash ? identifier(body.content_hash, "content_hash") : null;
    ...
    await client.query(
      "INSERT INTO artifacts(artifact_id,task_id,run_id,artifact_type,title,uri,summary,created_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8)",
      [row.artifact_id, row.task_id, row.run_id, row.artifact_type, row.title, row.uri, row.summary, row.created_at],
    );
    await appendRuntimeEvent(client, {
