...
function explicitFreeLocalProxyMode() {
  return String(process.env.AGENTOPS_DEPLOYMENT_MODE || "").trim().toLowerCase() === "free_local"
    && String(
    ...
    if (controlPlaneMode() === "proxy") {
      ...
      if (!explicitFreeLocalProxyMode()) {
        throw new ControlPlaneHttpError(
          ...
          "Python proxying requires explicit free_local deployment and proxy control-plane modes.",
        );
      }
