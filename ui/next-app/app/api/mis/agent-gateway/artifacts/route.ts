...
import { listAgentGatewayArtifacts } from "@/server/controlPlane/agentGatewayReadModels";
...
export async function GET(request: NextRequest) {
  try {
    if (controlPlaneMode() === "proxy") {
      return proxyFreeLocalRead(request, "/agent-gateway/artifacts");
    }
    const result = await listAgentGatewayArtifacts(request);
