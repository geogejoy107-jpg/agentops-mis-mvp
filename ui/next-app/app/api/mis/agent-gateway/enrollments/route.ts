...
import { listGatewayEnrollments } from "@/server/controlPlane/gatewayAdministration";
...
export async function GET(request: NextRequest) {
  try {
    if (controlPlaneMode() === "proxy") {
      return proxyFreeLocalRead(request, "/agent-gateway/enrollments");
    }
    const result = await listGatewayEnrollments(request);
