...
export async function POST(request: NextRequest) {
  try {
    await boundedJsonObject(request, { maxBytes: 1024, allowEmpty: true, label: "Human logout" });
    if (controlPlaneMode() !== "postgres") {
      return NextResponse.json(
        { ok: false, error: "human_session_postgres_required", message: "Human Session logout requires the TypeScript Postgres control plane.", token_omitted: true },
