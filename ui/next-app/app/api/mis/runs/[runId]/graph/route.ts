...
import { assertStrictReadQuery, getWorkspaceRunGraph } from "@/server/controlPlane/workspaceReadModels";
        ...
        return NextResponse.json(
          {
            ...
            message: "Production run graph reads require the TypeScript Postgres route.",
            token_omitted: true,
          },
          { status: 503, headers: PRIVATE_HEADERS },
        );
      }
      return freeLocalProxy(request, `/runs/${encodeURIComponent(runId)}/graph`);
    }
    assertStrictReadQuery(request.nextUrl.searchParams, []);
    const result = await getWorkspaceRunGraph(
      request.headers,
