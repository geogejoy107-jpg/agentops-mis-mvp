...
import { listWorkspaceTasks } from "@/server/controlPlane/workspaceReadModels";
    ...
    if (controlPlaneMode() === "proxy") {
      if (isProductionDeployment()) {
        return NextResponse.json(
          { ok: false, error: "typescript_route_owner_required", message: "Production task reads require the TypeScript Postgres route." },
          { status: 503, headers: { "Cache-Control": "no-store", Vary: "Cookie, X-AgentOps-Workspace-Id" } },
        );
      }
      return proxyControlPlaneRequest(request, "/tasks");
    }
