...
- Deployment contract: `deployment_readiness_v1` loads local readiness, backup/restore evidence, BYOC recovery drill status, signed audit export readiness, retention/export gates, SSO hooks, storage backend gates, and private connector policy read-only through the MIS API proxy; restore remains CLI-confirmed
