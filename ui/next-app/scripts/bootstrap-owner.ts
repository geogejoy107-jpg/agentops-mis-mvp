...
function parseArguments(argv: string[]) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument.startsWith("--password") || argument.startsWith("--pass")) {
      throw new BootstrapError(
        "password_argv_forbidden",
        "Password values are accepted only from a hidden prompt or --password-stdin.",
      );
    }
    if (!["--workspace-id", "--username", "--display-name"].includes(argument)) {
      throw new BootstrapError(
        "invalid_arguments",
        "Unknown or incomplete Owner bootstrap argument.",
      );
    }
    ...
    if (!value || value.startsWith("--")) {
      throw new BootstrapError(
        "invalid_arguments",
        "Owner bootstrap arguments require values.",
      );
    }
  ...
  if (!/^[A-Za-z0-9._:-]{1,128}$/.test(workspaceId)) {
    throw new BootstrapError(
      ...
      "--workspace-id must contain 1-128 safe identifier characters.",
    );
  }
  if (!/^[a-z0-9][a-z0-9._-]{2,63}$/.test(username)) {
    throw new BootstrapError(
      "username_invalid",
      "--username must contain 3-64 lowercase letters, digits, dot, underscore, or dash.",
    );
  }
...
async function readPasswordFromStdin() {
  ...
  if (lines.slice(1).some((line) => line.length > 0)) {
    throw new BootstrapError(
      "password_input_invalid",
      "--password-stdin accepts exactly one line.",
    );
  }
  return lines[0] || "";
}
...
async function bootstrap() {
  ...
  if (!dsn) {
    throw new BootstrapError(
      ...
      "AGENTOPS_POSTGRES_DSN is required.",
    );
  }
  const password = await suppliedPassword(args.passwordStdin);
  if (
    password.length < HUMAN_PASSWORD_MIN_LENGTH
    || password.length > HUMAN_PASSWORD_MAX_LENGTH
  ) {
    throw new BootstrapError(
      "password_length_invalid",
      `Password must contain ${HUMAN_PASSWORD_MIN_LENGTH}-${HUMAN_PASSWORD_MAX_LENGTH} characters.`,
    );
  }
  ...
  try {
    ...
    const credentialId =
      `hcred_${randomUUID().replaceAll("-", "").slice(0, 12)}`;
    ...
    await client.query(
      ...
      [`agentops-human-owner-bootstrap:${args.workspaceId}`],
    );
    const existingOwner = await client.query(
      `SELECT 1 FROM workspace_memberships
      WHERE workspace_id=$1 AND role='owner' LIMIT 1 FOR UPDATE`,
      [args.workspaceId],
    );
    if (existingOwner.rowCount) {
      throw new BootstrapError(
        "owner_already_initialized",
        "An Owner membership already exists in this workspace.",
      );
    }
    const existingUsername = await client.query(
      `SELECT 1 FROM human_login_credentials
      WHERE username=$1 LIMIT 1 FOR UPDATE`,
      [args.username],
    );
    if (existingUsername.rowCount) {
      throw new BootstrapError(
        "username_already_initialized",
        "The username is already initialized.",
      );
    }
    await client.query(
    ...
    await appendAudit(client as unknown as PoolClient, {
      ...
      metadata: {
        bootstrap_scope: "workspace_first_owner",
    ...
    if (error instanceof SchemaReadinessError) {
      throw new BootstrapError(
        error.code,
        "Owner bootstrap requires the current commercial Postgres schema.",
        1,
      );
    }
    throw new BootstrapError(
      "owner_bootstrap_failed",
      "Owner bootstrap failed closed; verify the migration and Postgres connectivity.",
      1,
    );
  } finally {
