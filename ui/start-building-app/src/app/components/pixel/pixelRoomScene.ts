...
type BlockOptions = Omit<Extract<PixelRoomProp, { kind: "block" }>, "kind" | "x" | "y" | "w" | "h" | "color">;

const block = (x: number, y: number, w: number, h: number, color: string, options: BlockOptions = {}): PixelRoomProp => ({
  kind: "block",
  x,
  y,
  w,
