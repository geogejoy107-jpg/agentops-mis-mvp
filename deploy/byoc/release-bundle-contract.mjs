...
function assertStaticCustomerBoundary() {
  ...
  if (
    ...
    || !workerEntrypoint.includes('"--import",')
    || !workerEntrypoint.includes('"tsx",')
    || workerEntrypoint.includes("tsx/dist/cli.mjs")
    ...
    || !workerEntrypoint.includes("stopping && !failureCode && !forcedStop && result.code === 0")
    || /result\.signal.*return 0/.test(workerEntrypoint)
    || !workerEntrypoint.includes("stopping && !failureCode && !forcedStop")
    || !workerEntrypoint.includes("worker_receipt_invalid")
    || !workerEntrypoint.includes("worker_stdout_limit_exceeded")
    || !workerEntrypoint.includes("worker_stderr_token_exposure")
    || !workerEntrypoint.includes("worker_health_state_write_failed")
    ...
    || !workerContainerAcceptance.includes("cleanupResources()")
    || !workerContainerAcceptance.includes("cleanup_confirmed: true")
    || !workerContainerAcceptance.includes("cleanup_secret_output_omitted: true")
    || !workerContainerAcceptance.includes("if (cleanupFailure)")
    || !workerContainerAcceptance.includes("JSON.stringify(successReceipt)")
...
try {
  ...
  run(
    process.execPath,
    [join(moduleDirectory, "openclaw-runtime-receipt-contract.mjs")],
    sourceRepository,
  );
