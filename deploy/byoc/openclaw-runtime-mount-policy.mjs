...
function validateMutableMounts(mutableMounts) {
  ...
  return mutableMounts.map((mount) => {
    if (
      ...
      || typeof mount !== "object"
      || Array.isArray(mount)
      || Object.keys(mount).sort().join(",") !== "kind,path,read_only"
    ...
    if (guestPath === "/" || mount.read_only !== POLICY[mount.kind].readOnly) {
      fail("runtime_mount_policy_mutable_mounts_invalid");
    }
    return Object.freeze({ kind: mount.kind, path: guestPath, read_only: mount.read_only });
  });
}
