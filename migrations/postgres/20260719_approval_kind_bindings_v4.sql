-- Make approval intent durable and enforce each approval kind against its
-- workspace-bound current-main subject graph. The migration runner owns the
-- transaction and exact receipt.
...
BEGIN
      ...
      EXISTS(
        ...
        WHERE audit.entity_type='approvals'
          ...
          AND audit.action IN (
            ...
            'workflow.run_execution_approval'
          )
  ...
  LOOP
    ...
    inferred_kind=CASE
      ...
      WHEN approval_record.has_customer_delivery_audit
        THEN 'customer_delivery'
    ...
    WHERE approval_id=approval_record.approval_id
      AND approval_kind IS DISTINCT FROM inferred_kind;
  END LOOP;
...
BEGIN
  IF NOT EXISTS (
    ...
    CHECK (
      (
        approval_kind IN ('tool_execution','prepared_action')
        AND tool_call_id IS NOT NULL
      )
      OR (
        approval_kind IN (
          'run_execution','agent_enrollment','customer_delivery'
        )
...
idx_agent_gateway_enrollment_approval_unique
ON agent_gateway_enrollment_requests(approval_id);

CREATE OR REPLACE FUNCTION
agentops_assert_approval_kind_binding(target_approval_id TEXT)
RETURNS VOID
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  IF NOT FOUND THEN
    IF EXISTS(
      ...
      RAISE EXCEPTION USING
        ERRCODE='23514',
        MESSAGE='approval_kind_binding_orphaned';
    END IF;
    RETURN;
  END IF;
  ...
  IF approval_record.task_workspace_id IS NULL
    ...
    OR approval_record.task_workspace_id<>
      approval_record.run_workspace_id
    ...
    OR (
      ...
      AND approval_record.requested_by_agent_id<>
        approval_record.run_agent_id
    ) THEN
    RAISE EXCEPTION USING
      ERRCODE='23514',
      MESSAGE='approval_kind_binding_invalid';
  END IF;
  ...
  SELECT
    ...
    COALESCE(bool_and(
      ...
      AND action.requested_by_agent_id=approval_record.run_agent_id
    ),TRUE)
  ...
  IF approval_record.approval_kind='prepared_action' THEN
    IF prepared_count<>1
      ...
      OR approval_record.tool_call_id IS NULL THEN
      RAISE EXCEPTION USING
        ERRCODE='23514',
        MESSAGE='approval_kind_binding_invalid';
    END IF;
  ELSIF approval_record.approval_kind='agent_enrollment' THEN
    IF prepared_count<>0
      ...
      OR approval_record.tool_call_id IS NOT NULL THEN
      RAISE EXCEPTION USING
        ERRCODE='23514',
        MESSAGE='approval_kind_binding_invalid';
    END IF;
  ELSIF approval_record.approval_kind='tool_execution' THEN
    IF prepared_count<>0
      ...
      RAISE EXCEPTION USING
        ...
        MESSAGE='approval_kind_binding_invalid';
    END IF;
  ELSIF approval_record.approval_kind IN (
    'run_execution','customer_delivery'
  ) THEN
    IF prepared_count<>0
      OR enrollment_count<>0
      OR approval_record.tool_call_id IS NOT NULL THEN
...
BEGIN
  ...
  IF OLD.approval_id IS DISTINCT FROM NEW.approval_id
    ...
    OR OLD.requested_by_agent_id IS DISTINCT FROM
      NEW.requested_by_agent_id THEN
    RAISE EXCEPTION USING
      ERRCODE='23514',
      MESSAGE='approval_binding_immutable';
  END IF;
  IF OLD.decision<>'pending'
    AND to_jsonb(OLD) IS DISTINCT FROM to_jsonb(NEW) THEN
    RAISE EXCEPTION USING
      ERRCODE='23514',
      MESSAGE='approval_terminal_immutable';
  END IF;
  IF OLD.decision='pending'
    ...
    RAISE EXCEPTION USING
      ERRCODE='23514',
      MESSAGE='approval_decision_state_invalid';
  END IF;
  IF OLD.decision='pending'
    AND NEW.decision<>'pending'
    AND (
...
BEGIN
  ...
  IF (
    TG_TABLE_NAME='tasks'
    AND (
      old_record->>'task_id' IS DISTINCT FROM
        new_record->>'task_id'
      OR old_record->>'workspace_id' IS DISTINCT FROM
        new_record->>'workspace_id'
    )
  ) OR (
    TG_TABLE_NAME='runs'
    AND (
      old_record->>'run_id' IS DISTINCT FROM
        new_record->>'run_id'
      OR old_record->>'task_id' IS DISTINCT FROM
        new_record->>'task_id'
      OR old_record->>'workspace_id' IS DISTINCT FROM
        new_record->>'workspace_id'
      OR old_record->>'agent_id' IS DISTINCT FROM
        new_record->>'agent_id'
    )
  ) OR (
    TG_TABLE_NAME='tool_calls'
    AND (
      old_record->>'tool_call_id' IS DISTINCT FROM
        new_record->>'tool_call_id'
      OR old_record->>'run_id' IS DISTINCT FROM
        new_record->>'run_id'
      OR old_record->>'agent_id' IS DISTINCT FROM
        new_record->>'agent_id'
    )
    ...
    RAISE EXCEPTION USING
      ERRCODE='23514',
      MESSAGE='approval_parent_binding_immutable';
  END IF;
  RETURN NEW;
END
$$;
...
BEGIN
  ...
  IF TG_OP='UPDATE'
    AND OLD.approval_id IS DISTINCT FROM NEW.approval_id THEN
    PERFORM agentops_assert_approval_kind_binding(OLD.approval_id);
  END IF;
...
BEGIN
  ...
  FOR approval_record IN
    ...
    JOIN runs run
      ...
      AND run.task_id=approval.task_id
    JOIN tasks task
    ...
    WHERE approval.approval_kind='customer_delivery'
      AND (
        (
          old_target_run_id IS NOT NULL
          AND approval.run_id=old_target_run_id
        )
        OR (
        ...
        OR (
          new_target_run_id IS NOT NULL
          AND approval.run_id=new_target_run_id
        )
        OR (
...
FOR EACH ROW
EXECUTE FUNCTION agentops_enforce_approval_kind_immutable();

DROP TRIGGER IF EXISTS
tasks_approval_parent_binding_immutable ON tasks;
CREATE TRIGGER tasks_approval_parent_binding_immutable
BEFORE UPDATE OF task_id,workspace_id ON tasks
FOR EACH ROW
EXECUTE FUNCTION agentops_enforce_approval_parent_binding_immutable();

DROP TRIGGER IF EXISTS
runs_approval_parent_binding_immutable ON runs;
...
DROP TRIGGER IF EXISTS
tool_calls_approval_parent_binding_immutable ON tool_calls;
...
DROP TRIGGER IF EXISTS
tool_calls_customer_delivery_evidence_sealed ON tool_calls;
CREATE TRIGGER tool_calls_customer_delivery_evidence_sealed
BEFORE INSERT OR UPDATE OR DELETE ON tool_calls
FOR EACH ROW
EXECUTE FUNCTION agentops_enforce_customer_delivery_evidence_seal();

DROP TRIGGER IF EXISTS
evaluations_customer_delivery_evidence_sealed ON evaluations;
...
DROP TRIGGER IF EXISTS
artifacts_customer_delivery_evidence_sealed ON artifacts;
...
DROP TRIGGER IF EXISTS
manifests_customer_delivery_evidence_sealed ON plan_evidence_manifests;
...
DROP TRIGGER IF EXISTS
agent_plans_customer_delivery_evidence_sealed ON agent_plans;
...
FOR EACH ROW
EXECUTE FUNCTION agentops_enforce_audit_log_append_only();
...
FOR EACH ROW
EXECUTE FUNCTION agentops_enforce_approval_kind_binding();

DROP TRIGGER IF EXISTS
prepared_actions_kind_binding_enforced ON prepared_actions;
...
BEGIN
  ...
  FOR approval_record IN
    ...
    FROM approvals
  LOOP
    PERFORM agentops_assert_approval_kind_binding(
